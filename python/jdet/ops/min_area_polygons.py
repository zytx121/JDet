...
def MinAreaPolygons(pointsets, polygons):
    src = f"""
    const int output_size = {polygons.numel()};
    const int num_pointsets = {pointsets.size(0)};
    min_area_polygons_cuda_kernel<<<GET_BLOCKS(output_size), THREADS_PER_BLOCK >>>(
                    num_pointsets, in0_p, out0_p);
    """
    return jt.code(polygons.shape,polygons.dtype,[pointsets,polygons],cuda_header=HEADER,cuda_src=src)
...
def min_area_polygons(pointsets):
    ...
    Returns:
        Tensor: Return the smallest polygons with shape (N, 8).
    """
    assert jt.flags.use_cuda
    polygons = jt.zeros((pointsets.size(0), 8), dtype=pointsets.dtype)
    return MinAreaPolygons(pointsets, polygons)
...
def test():
    ...
    assert np.allclose(
        min_area_polygons(pointsets).numpy(),
        expected_polygons,
        atol=1e-4)
