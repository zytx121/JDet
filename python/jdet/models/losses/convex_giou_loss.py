...
from jdet.ops.convex_giou import convex_giou
...
#             dim=1, keepdim=True).reshape(-1, 1)
#         pts_pred_mean = jt.cat([pts_pred_dx_mean, pts_pred_dy_mean], dim=-1)

#         pts_target_dx_mean = pts_target_all_dx.mean(
#             dim=1, keepdim=True).reshape(-1, 1)
#         pts_target_dy_mean = pts_target_all_dy.mean(
#             dim=1, keepdim=True).reshape(-1, 1)
#         pts_target_mean = jt.cat([pts_target_dx_mean, pts_target_dy_mean],
#                                     dim=-1)
